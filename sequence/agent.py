...
class InputCategory(BaseModel):
  category: Literal["fruit", "vegetable","other"]
...
@node(rerun_on_resume=True)
def select_fruit_vegetable(ctx: Context, node_input) -> str:
    category_obj = ctx.state.get("category")
    print("category_obj", category_obj) 
    
    # Get the category string safely from either a dict or Pydantic object
    category_val = None
    if isinstance(category_obj, dict):
        category_val = category_obj.get("category")
    elif hasattr(category_obj, "category"):
        category_val = category_obj.category

    # If the user's input was classified as a fruit, use the original input.
    # Otherwise, use the fruit name generated by generate_fruit_agent.
    if category_val == "fruit" or category_val == "vegetable":
        fruit = ctx.state.get("input")
    else:
        fruit = node_input
        
    ctx.state["selected_fruit_vegetable"] = fruit
    return fruit


generate_fruit_vegetable_agent = Agent(
    name="generate_fruit_vegetable_agent",
    instruction="""Return the name of a random fruit or vegetable.
      Return only the name, nothing else.""",
)

generate_benefit_agent = Agent(
    name="generate_benefit_agent",
    instruction="""Tell me a health benefit about the specified fruit or vegetable. {selected_fruit_vegetable}""",
)
